...
int CGameServer::OnRecv(SESSION_ID SessionID, CPacket * pPack)
{
	...
	// init
	CPacket *pSendPack = CPacket::Alloc();
	pSendPack->Clear();

	HEADER header = pPack->GetPayloadSize();

	// Payload
	__int64 i64Data = 0;
	*pPack >> i64Data;

	pSendPack->SetHeader((char*)&header);
	*pSendPack << i64Data;

	//보내는 SessionID 갯수만큼 RefCount++
	//pSendPack->RefIncrement();
	SendPacket(SessionID, pSendPack);
