			...
			do {
				if (iter != iterEnd) {
					...
					if (pSession && FD_ISSET(pSession->_sock, &readSet)) {
						...
						int len = recv(pSession->_sock, pSession->_sendQ.GetRearBufferPtr(), pSession->_sendQ.GetNotBrokenPutSize(), 0);
					...
					if (pSession && FD_ISSET(pSession->_sock, &writeSet)) {
						...
						if (!pSession->_sendQ.GetUseSize() == 0) {
							do {
								...
								int len = send(pSession->_sock, (char *)pSession->_sendQ.GetFrontBufferPtr(), pSession->_sendQ.GetNotBrokenGetSize(), 0);
								if (len == SOCKET_ERROR) {
									int err = WSAGetLastError();
									if (err == WSAEWOULDBLOCK)
										continue;
								...
								printf("[%d] send len: %d\n", pSession->_sock, len);
								pSession->_sendQ.MoveFront(len);
							} while (pSession->_sendQ.GetUseSize());
						}
